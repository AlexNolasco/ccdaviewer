...
@interface DummyClassParser : NSObject
@end

@implementation DummyClassParser
...
#define NSDateTimeAgoLocalizedStrings(key) NSLocalizedStringFromTableInBundle(key, @"NSDateTimeAgo", [NSBundle bundleWithPath:[[[NSBundle bundleForClass:[DummyClassParser class]] resourcePath] stringByAppendingPathComponent:@"NSDateTimeAgo.bundle"]], nil)
#endif
...
- (NSString *)dateTimeAgo
{
    ...
    NSDate *now = [NSDate date];
    NSDateComponents *components = [calendar components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekOfYear | NSCalendarUnitDay | NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond fromDate:self toDate:now options:0];

    if (components.year >= 1) {
        if (components.year == 1) {
            return LOCALIZED_STRING(@"1 year ago");
        }
        return [self stringFromFormat:@"%%d %@years ago" withValue:components.year];
    } else if (components.month >= 1) {
        if (components.month == 1) {
            return LOCALIZED_STRING(@"1 month ago");
        }
        return [self stringFromFormat:@"%%d %@months ago" withValue:components.month];
    } else if (components.weekOfYear >= 1) {
        if (components.weekOfYear == 1) {
            return LOCALIZED_STRING(@"1 week ago");
        }
        return [self stringFromFormat:@"%%d %@weeks ago" withValue:components.weekOfYear];
    } else if (components.day >= 1) // up to 6 days ago
    {
        if (components.day == 1) {
            return LOCALIZED_STRING(@"1 day ago");
        }
...
- (NSString *)dateTimeUntilNow
{
    ...
    if (components.hour >= 6) // if more than 6 hours ago, change precision
    {
        ...
        NSInteger endDay = [calendar ordinalityOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitEra forDate:now];
        ...
        if (diffDays == 0) // today!
        {
            ...
            if (startHourComponent.hour < 12 && endHourComponent.hour > 12) {
                return LOCALIZED_STRING(@"This morning");
            } else if (startHourComponent.hour >= 12 && startHourComponent.hour < 18 && endHourComponent.hour >= 18) {
                return LOCALIZED_STRING(@"This afternoon");
            }
            return LOCALIZED_STRING(@"Today");
        } else if (diffDays == 1) {
            return LOCALIZED_STRING(@"Yesterday");
        } else {
            NSInteger startWeek = [calendar ordinalityOfUnit:NSCalendarUnitWeekOfYear inUnit:NSCalendarUnitEra forDate:self];
            NSInteger endWeek = [calendar ordinalityOfUnit:NSCalendarUnitWeekOfYear inUnit:NSCalendarUnitEra forDate:now];
            NSInteger diffWeeks = endWeek - startWeek;
            if (diffWeeks == 0) {
                return LOCALIZED_STRING(@"This week");
            } else if (diffWeeks == 1) {
                return LOCALIZED_STRING(@"Last week");
            } else {
                NSInteger startMonth = [calendar ordinalityOfUnit:NSCalendarUnitMonth inUnit:NSCalendarUnitEra forDate:self];
                NSInteger endMonth = [calendar ordinalityOfUnit:NSCalendarUnitMonth inUnit:NSCalendarUnitEra forDate:now];
                NSInteger diffMonths = endMonth - startMonth;
                if (diffMonths == 0) {
                    return LOCALIZED_STRING(@"This month");
                } else if (diffMonths == 1) {
                    return LOCALIZED_STRING(@"Last month");
                } else {
                    NSInteger startYear = [calendar ordinalityOfUnit:NSCalendarUnitYear inUnit:NSCalendarUnitEra forDate:self];
                    NSInteger endYear = [calendar ordinalityOfUnit:NSCalendarUnitYear inUnit:NSCalendarUnitEra forDate:now];
                    NSInteger diffYears = endYear - startYear;
                    if (diffYears == 0) {
                        return LOCALIZED_STRING(@"This year");
                    } else if (diffYears == 1) {
                        return LOCALIZED_STRING(@"Last year");
                    }
                }
...
- (NSString *)stringFromFormat:(NSString *)format withValue:(NSInteger)value
{
    ...
    return [NSString stringWithFormat:LOCALIZED_STRING(localeFormat), value];
}
...
- (NSString *)timeAgoWithLimit:(NSTimeInterval)limit dateFormat:(NSDateFormatterStyle)dFormatter andTimeFormat:(NSDateFormatterStyle)tFormatter
{
    ...
    return [NSDateFormatter localizedStringFromDate:self dateStyle:dFormatter timeStyle:tFormatter];
}
